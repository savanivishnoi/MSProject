...
public class SensorsService extends Service implements SensorEventListener {
    ...
    public void onCreate() {
        ...
        mSensorManager.registerListener(this, heartRateSensor, 30000000 );//SensorManager.SENSOR_DELAY_NORMAL);
        ...
        Sensor accelerometer = mSensorManager.getDefaultSensor(SensorManager.SENSOR_ACCELEROMETER);
...
//        }
        ...
        for(int i = 0 ; i < sensorEvent.values.length; i++){
       //     Log.d("Mn Activity", "sensor type"+sensorEvent.sensor);
            Log.d("Mn Activity", "Value of 1 "+sensorEvent.values[i] +" sensor type "+sensorEvent.sensor);
        }
...
//        }
        ...
        if(sensorEvent.sensor.getType() == SensorManager.SENSOR_ACCELEROMETER){
            Log.d("Mn Activity", "  "+sensorEvent.values[0]+" "+sensorEvent.sensor.getName());
            for(int i = 0 ; i < sensorEvent.values.length; i++){
                Log.d("Mn Activity", "Value of 1"+sensorEvent.values[i]);
            }
